...
#
# Run from repo root:
#   ./docs/.blackbox/scripts/validate-loop.sh --auto-sync --interval-min 15
# Or from within docs/:
...
write_feature_research_telemetry() {
  ...
  python3 "$docs_root/.blackbox/scripts/research/collect_feature_research_metrics.py" \
  ...
  python3 "$docs_root/.blackbox/scripts/research/update_achievement_log.py" \
    --synth-plan "$synth_plan" \
    --write >/dev/null 2>&1 || true

  # Markdown mirrors (so ops visibility lives in .md, not just JSON).
  python3 "$docs_root/.blackbox/scripts/research/update_telemetry_md.py" \
    --synth-plan "$synth_plan" \
    --write >/dev/null 2>&1 || true

  # Gap-driven intelligence loop artifacts (MD-first).
  python3 "$docs_root/.blackbox/scripts/research/audit_intelligence_gaps.py" \
    --synth-plan "$synth_plan" \
    --write >/dev/null 2>&1 || true
...
while true; do
  ...
  code=0
  ok="false"
  if "$docs_root/.blackbox/scripts/validate-all.sh" "${args[@]}" >"$tmp" 2>&1; then
    ok="true"
  else
    code="$?"
    ok="false"
...

  {
    ...
    echo ""
	  ...
	  if [[ -n "$feature_research_synth_plan" ]]; then
	    ...
	    if "$docs_root/.blackbox/scripts/feature-research-health.sh" "${fr_args[@]}" >>"$log_file" 2>&1; then
	      feature_ok="true"
	    else
	      feature_ok="false"
	      failures=$((failures + 1))
	  ...
	  if [[ "$feature_ok" == "false" ]]; then
	    if [[ "$notify_local" == "true" && -x "$docs_root/.blackbox/scripts/notify-local.sh" ]]; then
	      "$docs_root/.blackbox/scripts/notify-local.sh" "[docs/.blackbox] feature research health failed (synth=${feature_research_synth_plan})"
	    fi
	    if [[ "$notify_telegram" == "true" && -x "$docs_root/.blackbox/scripts/notify-telegram.sh" ]]; then
	      "$docs_root/.blackbox/scripts/notify-telegram.sh" "[docs/.blackbox] feature research health failed (synth=${feature_research_synth_plan}) log=${log_file}"
	    fi
	  fi
	        ...
	        if command -v python3 >/dev/null 2>&1; then
	          {
	            ...
	            python3 "$docs_root/.blackbox/scripts/research/run_status.py" --synth-plan "$feature_research_synth_plan" --write || true
	            python3 "$docs_root/.blackbox/scripts/research/update_progress_dashboard.py" --synth-plan "$feature_research_synth_plan" --write || true
	            python3 "$docs_root/.blackbox/scripts/research/update_tranche_index.py" --synth-plan "$feature_research_synth_plan" --write || true
	            python3 "$docs_root/.blackbox/scripts/research/update_tranche_ledger.py" --synth-plan "$feature_research_synth_plan" --write || true
	  ...
	  # Optional: tranche audits (non-fatal; warnings only)
		  if [[ -n "$feature_research_synth_plan" && "$feature_research_audit_tranches" == "true" ]]; then
		    if command -v python3 >/dev/null 2>&1; then
		      ...
		      if [[ -n "$tranche_drafts_out" ]]; then
		        python3 "$docs_root/.blackbox/scripts/research/audit_tranche_drafts.py" --synth-plan "$feature_research_synth_plan" >"$tranche_drafts_out" 2>&1 || tranche_audit_code="$?"
		      else
		        python3 "$docs_root/.blackbox/scripts/research/audit_tranche_drafts.py" --synth-plan "$feature_research_synth_plan" >/dev/null 2>&1 || tranche_audit_code="$?"
		      fi

		      if [[ -n "$tranche_min_delta_out" ]]; then
		        python3 "$docs_root/.blackbox/scripts/research/validate_tranche_min_delta.py" --synth-plan "$feature_research_synth_plan" --last-n 5 >"$tranche_min_delta_out" 2>&1 || tranche_min_delta_code="$?"
		      else
		        python3 "$docs_root/.blackbox/scripts/research/validate_tranche_min_delta.py" --synth-plan "$feature_research_synth_plan" --last-n 5 >/dev/null 2>&1 || tranche_min_delta_code="$?"
		      fi
	      ...
	      if [[ "$feature_research_quality_gates" == "true" ]]; then
	        ...
	        if [[ "$feature_research_max_draft_tranches" =~ ^[0-9]+$ ]]; then
		      ...
		      audit_status_md="$docs_root/$feature_research_synth_plan/artifacts/tranche-audit-status.md"
		      {
		        ...
		        echo ""
		        echo "This file is updated by \`docs/.blackbox/scripts/validate-loop.sh\` when tranche audits are enabled."
		        echo ""
		        echo "## Summary"
		        echo ""
	      ...
	      if [[ "$feature_research_audit_notify" == "true" && "$tranche_warn" == "true" ]]; then
	        if [[ "$notify_local" == "true" && -x "$docs_root/.blackbox/scripts/notify-local.sh" ]]; then
	          "$docs_root/.blackbox/scripts/notify-local.sh" "[docs/.blackbox] tranche audit warnings (synth=${feature_research_synth_plan})"
	        fi
		        if [[ "$notify_telegram" == "true" && -x "$docs_root/.blackbox/scripts/notify-telegram.sh" ]]; then
		          "$docs_root/.blackbox/scripts/notify-telegram.sh" "[docs/.blackbox] tranche audit warnings (synth=${feature_research_synth_plan}) log=${log_file}"
		        fi
		      fi
	  ...
	  if [[ "$ok" == "false" ]]; then
	    if [[ "$notify_local" == "true" && -x "$docs_root/.blackbox/scripts/notify-local.sh" ]]; then
	      "$docs_root/.blackbox/scripts/notify-local.sh" "[docs/.blackbox] validate failed (exit=${code})"
	    fi
	    if [[ "$notify_telegram" == "true" && -x "$docs_root/.blackbox/scripts/notify-telegram.sh" ]]; then
      "$docs_root/.blackbox/scripts/notify-telegram.sh" "[docs/.blackbox] validate failed (exit=${code}) log=${log_file}"
    fi
  fi

  if [[ "$notify_recovery" == "true" && "$prev_ok" == "false" && "$ok" == "true" ]]; then
    if [[ "$notify_local" == "true" && -x "$docs_root/.blackbox/scripts/notify-local.sh" ]]; then
      "$docs_root/.blackbox/scripts/notify-local.sh" "[docs/.blackbox] validate recovered (OK)"
    fi
    if [[ "$notify_telegram" == "true" && -x "$docs_root/.blackbox/scripts/notify-telegram.sh" ]]; then
      "$docs_root/.blackbox/scripts/notify-telegram.sh" "[docs/.blackbox] validate recovered (OK)"
    fi
  fi
