...
#
# Run from repo root:
#   ./docs/.blackbox/scripts/new-tranche.sh --synth-plan docs/.blackbox/.plans/<synth> --type live-web
# Or from within docs/:
#   ./.blackbox/scripts/new-tranche.sh --synth-plan .blackbox/.plans/<synth> --type live-web
...
if [[ "$update_status" == "true" ]]; then
  if command -v python3 >/dev/null 2>&1; then
    ...
    python3 "$docs_root/.blackbox/scripts/research/run_status.py" --synth-plan "$synth_plan" --write
  else
...
if [[ "$update_kpis" == "true" ]]; then
  if command -v python3 >/dev/null 2>&1; then
    ...
    python3 "$docs_root/.blackbox/scripts/research/update_progress_dashboard.py" --synth-plan "$synth_plan" --write
  else
...
if [[ "$update_index" == "true" ]]; then
  if command -v python3 >/dev/null 2>&1; then
    ...
    python3 "$docs_root/.blackbox/scripts/research/update_tranche_index.py" --synth-plan "$synth_plan" --write
  else
