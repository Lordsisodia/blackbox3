...
#
# Run from repo root:
#   ./docs/.blackbox/scripts/start-oss-discovery-cycle.sh
# Or from within docs/:
...
run_id="$(date +%H%M%S)"
created="$("$docs_root/.blackbox/scripts/new-run.sh" oss-discovery "github oss discovery cycle ${run_id}" \
...
if [[ "$enable_derived_queries" == "true" ]]; then
  ...
  python3 "$docs_root/.blackbox/scripts/research/generate_oss_query_bank.py" \
...
if [[ "$enable_query_rotation" == "true" ]]; then
  ...
  python3 "$docs_root/.blackbox/scripts/research/rotate_query_bank.py" \
    --in "$combined_queries" \
    --out "$rotated_queries" \
    --state "$state_file" \
...
python3 "$docs_root/.blackbox/scripts/research/github_search_repos.py" \
  --out "$search_report" \
...
if [[ "$enable_gap_boost" == "true" ]]; then
  ...
  python3 "$docs_root/.blackbox/scripts/research/export_oss_candidates.py" \
    --search-json "$search_json" \
    --out-json "$plan_path/artifacts/extracted.search.json"

  python3 "$docs_root/.blackbox/scripts/research/suggest_oss_gap_queries.py" \
  ...
  if [[ "$gap_query_lines" == "0" ]]; then
    ...
    python3 "$docs_root/.blackbox/scripts/research/merge_search_extracted.py" \
      --in "$search_json" \
      --in "$search_json_gap" \
...
if is_true "$enable_catalog_gap_boost" && [[ -f "$catalog_file" ]] && [[ "$repo_line_count" -lt "$catalog_gap_boost_min_tranche_repos" ]]; then
  ...
  python3 "$docs_root/.blackbox/scripts/research/generate_oss_catalog_gap_queries.py" \
...
PY)"
  ...
  python3 "$docs_root/.blackbox/scripts/research/select_gap_queries_subset.py" \
...
if [[ "$repo_line_count" == "0" ]]; then
  ...
  python3 "$docs_root/.blackbox/scripts/research/summarize_oss_coverage.py" \
  ...
  python3 "$docs_root/.blackbox/scripts/research/summarize_oss_auto_picks.py" \
    --extracted-json "$plan_path/artifacts/extracted.json" \
    --out "$plan_path/artifacts/auto-picks.md"

  python3 "$docs_root/.blackbox/scripts/research/generate_oss_deepening_queue_from_extracted.py" \
    --extracted-json "$plan_path/artifacts/extracted.json" \
...
EOF
  ...
  mkdir -p "$catalog_dir"
  python3 "$docs_root/.blackbox/scripts/research/update_oss_catalog.py" \
    --run-path "$plan_path" \
    --extracted-json "$plan_path/artifacts/extracted.json" \
    --out-catalog "$catalog_dir/catalog.json" \
    --out-index "$catalog_dir/index.md" || echo "WARN: failed to update OSS catalog (non-fatal)" >&2
  "$docs_root/.blackbox/scripts/render-oss-catalog.sh" --catalog-dir ".blackbox/oss-catalog" \
    || echo "WARN: failed to render OSS catalog artifacts (non-fatal)" >&2

  if [[ -x "$docs_root/.blackbox/scripts/new-step.sh" ]]; then
    if [[ -n "$search_blocked_reason" || -n "$search_error_http_status" ]]; then
      "$docs_root/.blackbox/scripts/new-step.sh" --plan "$plan_path" "OSS discovery cycle blocked (GitHub Search API rate limit)"
    else
      "$docs_root/.blackbox/scripts/new-step.sh" --plan "$plan_path" "OSS discovery cycle completed (empty tranche)"
    fi
  fi
...
python3 "$docs_root/.blackbox/scripts/research/fetch_github_repos.py" \
  --input "$repo_list" \
...
./.blackbox/scripts/start-oss-discovery-cycle.sh --tranche-name ${tranche_name}
\`\`\`
...
"$docs_root/.blackbox/scripts/render-oss-catalog.sh" --catalog-dir ".blackbox/oss-catalog" \
  ...
  if [[ -x "$docs_root/.blackbox/scripts/new-step.sh" ]]; then
    "$docs_root/.blackbox/scripts/new-step.sh" --plan "$plan_path" "OSS discovery cycle blocked (GitHub rate limit)"
  fi
...
python3 "$docs_root/.blackbox/scripts/research/summarize_oss_meta.py" \
  --entries-dir "$plan_path/oss/entries" \
  --out "$plan_path/artifacts/oss-triage.md"

python3 "$docs_root/.blackbox/scripts/research/build_oss_shortlist.py" \
  --entries-dir "$plan_path/oss/entries" \
  --out "$plan_path/artifacts/oss-shortlist.md" \
  --top 25

python3 "$docs_root/.blackbox/scripts/research/score_oss_from_metadata.py" \
  --entries-dir "$plan_path/oss/entries" \
...
- [ ] Promote best learnings if reusable (optional): \`./.blackbox/scripts/promote.sh <plan> <slug>\`
EOF
...
if [[ -x "$docs_root/.blackbox/scripts/new-step.sh" ]]; then
  "$docs_root/.blackbox/scripts/new-step.sh" --plan "$plan_path" "OSS discovery cycle completed"
fi
