...
#
# Why:
# - check-blackbox.sh enforces that live scripts/templates match _template
# - During iteration it's easy to update only one side
# - This script makes it a 1-command fix: live -> _template
#
# Run from repo root:
#   ./docs/.blackbox/scripts/sync-template.sh
# Or from within docs/:
...
if [[ "$prune" == "true" ]]; then
  ...
  for f in "$dst/scripts/"*; do
    [[ -f "$f" ]] || continue
    base="$(basename "$f")"
    if [[ ! -f "$src/scripts/$base" ]]; then
      maybe_delete "$f"
    fi
  done
...
extra_scripts="$(find "$dst/scripts" -maxdepth 1 -type f -print | while read -r p; do b="$(basename "$p")"; [[ -f "$src/scripts/$b" ]] || echo "$b"; done)"
...
echo "Done."
echo "Next: run ./docs/.blackbox/scripts/check-blackbox.sh to verify."
