...
#
# Run from repo root:
#   ./docs/.blackbox/scripts/start-agent-cycle.sh --hours 8 --prompts 50 "my goal"
# Or from within docs/:
#   ./.blackbox/scripts/start-agent-cycle.sh --hours 8 --prompts 50 "my goal"
...
if [[ -n "$plan_override" ]]; then
  ...
  created="$("$docs_root/.blackbox/scripts/new-plan.sh" "$goal")"
...
- Every ${checkpoint_every} prompt(s), add a step file:
  - \`./.blackbox/scripts/new-step.sh --plan ${plan_rel} "Checkpoint: <what changed>"\`

If context gets long:
- \`./.blackbox/scripts/compact-context.sh --plan ${plan_rel}\`
