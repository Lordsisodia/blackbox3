...
write_start_here() {
...

  {
    ...
    echo ""
    echo '```bash'
    echo "./docs/.blackbox/scripts/new-step.sh --plan ${plan_path} \"Checkpoint: <what changed>\""
    echo '```'
    echo ""
    echo "Compact early if context grows:"
    echo ""
    echo '```bash'
    echo "./docs/.blackbox/scripts/compact-context.sh --plan ${plan_path}"
    echo '```'
    echo ""
...
\`\`\`bash
python3 docs/.blackbox/scripts/validate-feature-research-run.py --plan __STEP1_PLAN__ --kind step-01
python3 docs/.blackbox/scripts/validate-feature-research-run.py --plan __STEP2_PLAN__ --kind step-02
python3 docs/.blackbox/scripts/validate-feature-research-run.py --plan __STEP3_PLAN__ --kind step-03
python3 docs/.blackbox/scripts/validate-feature-research-run.py --plan __STEP4_PLAN__ --kind step-04
python3 docs/.blackbox/scripts/validate-feature-research-run.py --plan __SYNTH_PLAN__ --kind synthesis
\`\`\`

## 🔎 Evidence Index (optional auto-generation)

\`\`\`bash
python3 docs/.blackbox/scripts/research/generate_evidence_index.py --synth-plan __SYNTH_PLAN__
\`\`\`

## 📊 Run status dashboard (fast progress check)

\`\`\`bash
python3 docs/.blackbox/scripts/research/run_status.py --synth-plan __SYNTH_PLAN__ --write
\`\`\`
...
echo "  ./docs/.blackbox/scripts/new-step.sh --plan docs/.blackbox/.plans/<plan> \"Checkpoint: <what changed>\""
echo ""

if [[ -n "$target_user_first" && -n "$license_policy" ]]; then
  echo "⚙️ Applying decisions across all plans..."
  python3 "$docs_root/.blackbox/scripts/research/set_feature_research_config.py" \
    --synth-plan "$synth_plan" \
    --target-user-first "$target_user_first" \
    --license-policy "$license_policy"
  echo ""
  echo "✅ Validating decisions (no TBD)..."
  python3 "$docs_root/.blackbox/scripts/research/validate_feature_research_config.py" \
    --synth-plan "$synth_plan"
  echo ""
