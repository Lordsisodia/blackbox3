...
fix_perms() {
  if [[ -x "$docs_root/.blackbox/scripts/fix-perms.sh" ]]; then
    "$docs_root/.blackbox/scripts/fix-perms.sh" >/dev/null 2>&1 || true
  fi
}

run_validators() {
  if command -v python3 >/dev/null 2>&1; then
    python3 "$docs_root/.blackbox/scripts/validate-docs.py"
...
run_vendor_leaks() {
  ...
  local script="$docs_root/.blackbox/scripts/check-vendor-leaks.sh"
  if [[ ! -x "$script" ]]; then
...
if [[ "$auto_sync" != "true" ]]; then
  ...
  echo "  ./.blackbox/scripts/sync-template.sh" >&2
  echo "If this is a permission issue, run:" >&2
  echo "  ./.blackbox/scripts/fix-perms.sh" >&2
  exit 1
fi
...
if [[ ${#sync_args[@]} -gt 0 ]]; then
  "$docs_root/.blackbox/scripts/sync-template.sh" "${sync_args[@]}"
else
  "$docs_root/.blackbox/scripts/sync-template.sh"
fi
fix_perms
check
run_validators
