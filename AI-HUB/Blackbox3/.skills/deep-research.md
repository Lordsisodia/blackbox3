...
## Outputs (artifacts)
1. A **plan folder** saved in `.plans/`
2. (Optional) A **research note** saved in `.knowledge/deepresearch/` if the work produces reusable knowledge
3. (Required) Run artifacts captured under the plan folder (recommended: `artifacts/`)
4. (Required) A knowledge ledger entry if anything is written outside `.blackbox/`
...
### 2) Create the plan file (before doing the work)
Create a new plan folder in `.plans/` using the new-plan.sh script:
```bash
./scripts/new-plan.sh "research-topic"
```
...
- Save a short evergreen write-up in `.knowledge/deepresearch/`
- Link it from the plan under "Artifacts"

### 6) Organize and route deliverables (required)
If you create/update knowledge artifacts outside `.blackbox/`:
- Organize in appropriate `.knowledge/` subdirectories
- Update `.knowledge/LEDGER.md` if it exists
