...
export function HelpCollectionScreen({ collection }: HelpCollectionScreenProps) {
  return (
    <HelpShell title={`${collection.title} help`} breadcrumbs={[
      ...
      <HelpCollectionHero collection={collection} />
      <div className="space-y-3">
        {collection.articles.map((article) => (
          <Link
            key={article.slug}
            href={`/partners/community/help/${collection.slug}/${article.slug}`}
            className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-left text-white transition hover:border-siso-orange/70"
          >
            <div>
...
export function HelpArticleScreen({ collection, article }: HelpArticleScreenProps) {
  return (
    ...
    >
      <article className="space-y-6 rounded-3xl border border-white/10 bg-white/[0.04] p-5 text-sm text-siso-text-muted">
        <header>
        ...
        <div className="space-y-5 text-sm text-siso-text-muted">
        ...
        <footer className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-black/40 p-4 text-xs text-siso-text-muted md:flex-row md:items-center md:justify-between">
    ...
    <SettingsDetailLayout wrapContent={false} compactHeader hideHeader srTitle={title} backHref={null}>
      <div className="relative space-y-6">
        <Link
          ...
          className="absolute right-0 -top-2 inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/30 text-white shadow-[0_6px_18px_rgba(0,0,0,0.45)] backdrop-blur transition hover:border-siso-orange hover:text-siso-orange"
        >
          <Menu className="h-5 w-5" />
        </Link>
        {breadcrumbs && <HelpBreadcrumbs items={breadcrumbs} />}
        {children}
      </div>
...
const iconMap: Record<HelpCollectionIcon, React.ComponentType<{ className?: string }>> = {
  "life-buoy": LifeBuoy,
...
function HelpCollectionCard({ collection }: { collection: HelpCollection }) {
  ...
  return (
    <Link
      ...
      className="flex items-center justify-between gap-3 rounded-3xl border border-white/10 bg-white/[0.04] p-4 text-white transition hover:border-siso-orange/70"
    >
...
function HelpCollectionHero({ collection }: { collection: HelpCollection }) {
  const Icon = resolveIcon(collection.icon);
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4 text-white">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-siso-text-muted">All collections ▸ {collection.title}</p>
