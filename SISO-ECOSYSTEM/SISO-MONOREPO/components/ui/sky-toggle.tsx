...
type SwitchProps = {
  ...
  className?: string;
  /**
   * Scale relative to the default size (30px). 0.5 ≈ 50% smaller.
   */
  scale?: number;
};

const Switch = ({ checked, onChange, className, scale = 0.7 }: SwitchProps) => {
  return (
    <StyledWrapper
      className={className}
      style={{ ["--toggle-size" as any]: `${30 * scale}px` }}
    >
...
const StyledWrapper = styled.div`
  ...
  .theme-switch, .theme-switch *, .theme-switch *::before, .theme-switch *::after {
    ...
    margin: 0;
    padding: 0;
    font-size: var(--toggle-size, 30px);
  }
  ...
  .theme-switch__clouds {
    width: 1.25em; height: 1.25em; background-color: var(--clouds-color); border-radius: var(--container-radius); position: absolute; bottom: -0.625em; left: 0.312em;
